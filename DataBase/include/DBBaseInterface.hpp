...
public:
	...
	void SetCharSet(const std::string& szCharSet);       //设置字符集
	void SetTimeOut(unsigned int iTimeOut);         //设置超时
	...
	const std::string& GetCharSet() const;          //获取字符集
	const unsigned int& GetTimeOut() const;    //获取超时

	void SetIsAutoCommit(bool bIsAutoCommit); //设置是否自动提交
	bool GetIsAutoCommit();                   //获取是否自动提交
	void SetIsConnect(bool bIsConnect);       //设置是否已经连接
	bool GetIsConnect();                      //获取是否已经连接
	...
	virtual bool Connect(bool bAutoCommit) = 0; //连接数据库
	...
	virtual bool BindParm(unsigned int iIndex, long long& value, short& indp, bool bUnsigned = false) = 0;
...
private:
	...
	virtual bool SetSqlState(bool bIsStmt = false) = 0; //设置SQL执行状态
...
public:
	...
	static bool ConvertBinaryToHex(const unsigned char* binary, unsigned long long binary_len, std::string& szOutHex); //二进制数据转十六进制字符串
	static bool ConvertHexToBinary(const std::string& szInHex, unsigned char* binary, unsigned long long& binary_len); //十六进制字符串转二进制数据
...
protected:
	...
	std::string    m_szCharSet;      //字符集
	unsigned       m_iTimeOut;       //超时，秒
